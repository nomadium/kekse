...
module Kekse
  class Challenge
    ...
    Signature = Struct.new(:message, :bytes) do
      ...
      def valid_signature?
        !bytes.nil? && known_key? && ssh_signature.verify(message.to_s)
      end
      ...
      def self.invalid_signature
        InvalidSignature.new(nil, nil)
      end
    end

    InvalidSignature = Struct.new(:message, :bytes) do
      def invalid?
        true
      end
    end
